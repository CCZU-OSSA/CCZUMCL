...
impl TaskReporter {
  pub fn report_started(&self, total: i64) {
...
pub struct GroupReporter<'a> {
  app: AppHandle,
  group: &'a str,
}

impl<'a> GroupReporter<'a> {
  pub fn new(app: AppHandle, group: &'a str) -> Self {
    Self { app, group }
  }
  pub fn report_group_started(&self) {
    GroupEvent::emit_group_started(&self.app, self.group);
  }

  pub fn report_group_failed(&self) {
    GroupEvent::emit_group_failed(&self.app, self.group);
  }

  pub fn report_group_completed(&self) {
    GroupEvent::emit_group_completed(&self.app, self.group);
  }

  pub fn report_group_cancelled(&self) {
    GroupEvent::emit_group_cancelled(&self.app, self.group);
  }

  pub fn report_group_stopped(&self) {
